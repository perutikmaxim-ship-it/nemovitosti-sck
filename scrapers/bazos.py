...
class BazosScraper(BaseScraper):
    ...
    def _build_url(self, location: str) -> str:
        """Sestaví URL pro vyhledávání - pouze PRODEJ bytů."""
        encoded_location = quote(location)
        # reality.bazos.cz/byty/ = prodej i pronájem dohromady
        # Filtrujeme přes ?category=prodej nebo přímo podkategorií
        return (
            f"{BASE_URL}/byty/"
    ...
    def _parse_item(self, item, location: str) -> Optional[dict]:
        ...
        try:
            ...
            RENTAL_KEYWORDS = ["pronájem", "pronajmu", "nájem", "podnájem", "k pronájmu", "rent"]
            title_lower = title.lower()
            if any(kw in title_lower for kw in RENTAL_KEYWORDS):
                return None
